...
func TestGethAddresses(t *testing.T) {
	// We test if the addresses in geth match those in umi-bindings, to avoid an import-cycle:
	// we import geth in the monorepo, and do not want to import umi-bindings into geth.
	require.Equal(t, L1BlockAddr, types.L1BlockAddr)
}

// TestL1BlockSlots ensures that the storage layout of the L1Block
// contract matches the hardcoded values in `umi-geth`.

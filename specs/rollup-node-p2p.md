...
The umi-node implements a similar request-response encoding for its sync protocols as the L1 ethereum Beacon-Chain.
...
The req-resp protocols served by the umi-node all have `/ProtocolPrefix` set to `/umi2tack/req`.
...
Protocol ID: `/umi2tack/req/payload_by_number/<chain-id>/0/`

- `/MessageName` is `/block_by_number/<chain-id>` where `<chain-id>` is set to the umi-node L2 chain ID.
- `/SchemaVersion` is `/0`

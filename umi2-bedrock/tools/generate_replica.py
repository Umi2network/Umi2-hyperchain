...
parser.add_argument('--umi-node-tag', type=str, help='docker tag to use with the rollup node', default='develop')
parser.add_argument('--umi-node-http-port', type=int, help='rollup node http port', default=9545)
parser.add_argument('--umi-node-metrics-port', type=int, help='rollup node http port', default=7300)
parser.add_argument('--umi-node-pprof-port', type=int, help='rollup node http port', default=6300)
...
def main():
    ...
    dump_file(outdir, 'umi-node.env', op_node_env_tmpl(l1_rpc, f'ws://l2:{args.geth_ws_port}', args.op_node_http_port))
...
services:
  ...
  umi-node:
    depends_on:
      - l2
    image: us-central1-docker.pkg.dev/bedrock-goerli-development/images/umi-node:{op_node_tag}
    command: umi-node
    ports:

...
// TestMissingGasLimit tests that umi-geth cannot build a block without gas limit while optimism is active in the chain config.
...
// TestTxGasSameAsBlockGasLimit tests that umi-geth rejects transactions that attempt to use the full block gas limit.

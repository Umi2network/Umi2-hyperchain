...
We’re going to be spinning up an EVM Rollup from the OP Stack source code.  You could use docker images, but this way we keep the option to modify component behavior if you need to do so. The OP Stack source code is split between two repositories, the [Optimism Monorepo](https://github.com/ethereum-optimism/optimism) and the [`umi-geth`](https://github.com/ethereum-optimism/umi-geth) repository.
...
1. Install required modules. This is a slow process, while it is running you can already start building `umi-geth`, as shown below.
...
1. Build the various packages inside of the Optimism Monorepo.
    ...
    ```bash
    make umi-node umi-batcher umi-proposer
    yarn build
    ```

### Build umi-geth

1. Clone [`umi-geth`](https://github.com/ethereum-optimism/umi-geth):

    ```bash
    cd ~
    git clone https://github.com/ethereum-optimism/umi-geth.git
    ```
...
We’ve set up the L1 side of things, but now we need to set up the L2 side of things. We do this by generating three important files, a `genesis.json` file, a `rollup.json` configuration file, and a `jwt.txt` [JSON Web Token](https://jwt.io/introduction) that allows the `umi-node` and `umi-geth` to communicate securely.

1. Head over to the `umi-node` package:
...
1. Run the following command, and make sure to replace `<RPC>` with your L1 RPC URL:
    ...
    You should then see the `genesis.json` and `rollup.json` files inside the `umi-node` package.
...
1. Finally, we’ll need to copy the `genesis.json` file and `jwt.txt` file into `umi-geth` so we can use it to initialize and run `umi-geth` in just a minute:

    ```bash
    cp genesis.json ~/umi-geth
    cp jwt.txt ~/umi-geth
    ```

## Initialize umi-geth

We’re almost ready to run our chain! Now we just need to run a few commands to initialize `umi-geth`. We’re going to be running a Sequencer node, so we’ll need to import the `Sequencer` private key that we generated earlier. This private key is what our Sequencer will use to sign new blocks.

1. Head over to the `umi-geth` repository:
...
1. Import the key into `umi-geth`:

    ```bash
    ./build/bin/geth account import --datadir=datadir --password=datadir/password datadir/block-signer-key
    ```

1. Next we need to initialize `umi-geth` with the genesis file we generated and copied earlier:
...
The first two, `umi-geth` and `umi-node`, have to run on every node.
The other two, `umi-batcher` and `umi-proposer`, run only in one place, the sequencer that accepts transactions.
...
Run `umi-geth` with the following commands.
...
And `umi-geth` should be running! You should see some output, but you won’t see any blocks being created yet because `umi-geth` is driven by the `umi-node`. We’ll need to get that running next.
...
- The `umi-proposer` requires access to the full state.
  If at some point `umi-proposer` needs to look beyond 256 blocks in the past (8.5 minutes in the default configuration), for example because it was down for that long, we need archive mode.

- The [explorer](./explorer.md) requires archive mode.

:::

#### Reinitializing umi-geth

There are several situations are indicate database corruption and require you to reset the `umi-geth` component:

- When `umi-node` errors out when first started and exits.
- When `umi-node` emits this error:
...
Once we’ve got `umi-geth` running we’ll need to run `umi-node`. Like Ethereum, the OP Stack has a consensus client (the `umi-node`) and an execution client (`umi-geth`). The consensus client drives the execution client over the Engine API.
...
Once you run this command, you should start seeing the `umi-node` begin to process all of the L1 information after the starting block number that you picked earlier. Once the `umi-node` has enough information, it’ll begin sending Engine API payloads to `umi-geth`. At that point, you’ll start to see blocks being created inside of `umi-geth`. We’re live!


::: tip Peer to peer synchronization

If you use a chain ID that is also used by others, for example the default (42069), your `umi-node` will try to use peer to peer to speed up synchronization.
...
The `umi-batcher` takes transactions from the Sequencer and publishes those transactions to L1. Once transactions are on L1, they’re officially part of the Rollup. Without the `umi-batcher`, transactions sent to the Sequencer would never make it to L1 and wouldn’t become part of the canonical chain. The `umi-batcher` is critical!
...
Now start `umi-proposer`, which proposes new state roots.

```bash
cd ~/optimism/umi-proposer
...
If `umi-geth` aborts (for example, because the computer it is running on crashes), you might get these errors on `umi-node`:
...
If `umi-batcher` runs out of ETH, it cannot submit write new transaction batches to L1.

...
func (d *Downloader) Synchronise(id string, head common.Hash, td *big.Int, mode SyncMode) error {
	...
	case errTimeout, errBadPeer, errStallingPeer, errUnsyncedPeer,
		...
		if d.dropPeer == nil {
			...
			log.Warn("Downloader wants to drop peer, but peerdrumi-function is not set", "peer", id)
		} else {
			d.dropPeer(id)
		}
	default:
	...
	for {
		...
		case <-timeout.C:
			if d.dropPeer == nil {
				// The dropPeer method is nil when `--copydb` is used for a local copy.
				// Timeouts can occur if e.g. compaction hits at the wrong time, and can be ignored
				p.log.Warn("Downloader wants to drop peer, but peerdrumi-function is not set", "peer", p.id)
				break
			}
				...
				if peer := d.peers.Peer(pid); peer != nil {
					...
					if fails > 2 {
						...
						if d.dropPeer == nil {
							// The dropPeer method is nil when `--copydb` is used for a local copy.
							// Timeouts can occur if e.g. compaction hits at the wrong time, and can be ignored
							peer.log.Warn("Downloader wants to drop peer, but peerdrumi-function is not set", "peer", pid)
						} else {
							d.dropPeer(pid)
